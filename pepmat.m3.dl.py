...
# 在循环内用正则表达式从dats文件夹中读取shuf数据文件，每个循环处理一个数据，输出一个报告文件
# 使用简单的深度卷积方法，通过1*1卷积核或等大小的卷积核卷积，之后使用分类器
# 获得对二维特征的分类结果
# 含量需要二维特征与标签进行映射会回归建模
# 调用过程中，直接用模型给出对二维数据的分类结果过、估计的含量信息
...
train_dat, test_dat, train_cls, test_cls = train_test_split(dat, cls,
                                                            test_size=0.2,
                                                            random_state=11)
train_dat = np.expand_dims(train_dat, axis=3)
test_dat = np.expand_dims(test_dat, axis=3)

# 建立模型
model_m3_cls = Sequential()
model_m3_cls.add(
    Conv2D(32, (3, 3), activation='relu', input_shape=(10, 10, 1)))
model_m3_cls.add(Conv2D(32, (1, 1), activation='relu'))
model_m3_cls.add(MaxPooling2D((2, 2)))
model_m3_cls.add(Conv2D(64, (3, 3), activation='relu'))
model_m3_cls.add(Conv2D(64, (1, 1), activation='relu'))
model_m3_cls.add(MaxPooling2D((2, 2)))
model_m3_cls.add(Flatten())
model_m3_cls.add(Dense(64, activation='relu'))
model_m3_cls.add(Dense(15, activation='softmax'))  # 最终转为15个值用于分类
# model_m3_cls.summary()

# 编译模型
model_m3_cls.compile(loss='sparse_categorical_crossentropy',
                     optimizer='adam', metrics=['accuracy'])
history = model_m3_cls.fit(train_dat, train_cls,
                           epochs=15, batch_size=144,
                           validation_data=(test_dat, test_cls))

# 保存模型
model_m3_cls.save('models-saved/model_m3_cls')

# 评估模型
score = model_m3_cls.evaluate(test_dat, test_cls, verbose=0)
print(f'Accuracy: {score[1]}')  # 1.0
...
# 搭建模型
model_m3_reg = tf.keras.Sequential([
...
# 训练模型
model_m3_reg.compile(loss='mse', optimizer='adam')
model_m3_reg.fit(train_dat, train_val, epochs=10, batch_size=144)
# 600 数据规模 -> 48
# 1800 数据规模 -> 144
# 6000 数据规模 -> 480

# 保存模型
model_m3_reg.save('models-saved/model_m3_reg')

# 模型评估
y_pred = model_m3_reg.predict(test_dat)
r2 = r2_score(test_val, y_pred).round(4)
print("R2 Score:", r2)  # 0.9926(2层), 0.9958(4层) （越接近1，预测能力越好）
mse = mean_squared_error(test_val, y_pred).round(4)
print("Mean Squared Error:", mse)  # 3.9234(2层), 2.1636(4层) （越小，预测结果越准）
